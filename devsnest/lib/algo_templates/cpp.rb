  ...
  class CPP < Templates::BaseHelper
    ...
    def input_builder(name, datastructure, dtype, dependent)
      meta = {
        ...
        'binary_tree' => {
          'int' => ["string #{name}_raw_str;", "getline(cin, #{name}_raw_str);", "stringstream #{name}_raw_str_stream(#{name}_raw_str);", "vector<int> #{name}_raw_array;", "while(#{name}_raw_str_stream >> #{name}_raw_str){", "#{name}_raw_array.push_back(#{name}_raw_str);" ,"}", "#{name} = #{name}->createBinaryTree(#{name}_raw_array);"]
        }
      }

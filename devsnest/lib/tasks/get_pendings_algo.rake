  ...
  task run: :environment do
    ...
    pending_submissions_batches.each do |pending_submissions|
      pending_submissions.each do |submission|
        submission[:test_cases].each do |token, _stats|
          poll = HTTParty.get(ENV['JUDGEZERO_URL'] + "/submissions/#{token}?base64_encoded=true", headers: jz_headers)
          begin

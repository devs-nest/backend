...
MassRoleModifierWorker.perform_async('add_role', discord_ids, 'TEAM NAME')
MassRoleModifierWorker.perform_async('delete_role', discord_ids, 'TEAM NAME')
...
Group.where(version:2).each do |group|
    ...
    MassRoleModifierWorker('add_role', discord_ids, group.name)
end
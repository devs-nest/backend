...
RSpec.describe Challenge, type: :model do
  context 'algo template specs' do
    ...
    context 'check competetive leaderboard' do
      ...
      let!(:spec_leaderboard) { Leaderboard.new('dn_leaderboard-test', Devsnest::Application::REDIS_OPTIONS, { redis_connection: Redis.new(url: 'redis://127.0.0.1:6379/0') }) }
      let!(:ch_leaderboard) { question.generate_leaderboard }
      let!(:u_s1) { create(:algo_submission, user_id: user.id, challenge_id: question.id, passed_test_cases: 7, total_test_cases: 10, is_best_submission: true, is_submitted: true, status: "Accepted") }
      let!(:u3_s1) { create(:algo_submission, user_id: user3.id, challenge_id: question.id, passed_test_cases: 7, total_test_cases: 10, is_best_submission: true, is_submitted: true, status: "Accepted") }
      let!(:u_s2) { create(:algo_submission, user_id: user.id, challenge_id: question.id, passed_test_cases: 10, total_test_cases: 10, is_best_submission: false, is_submitted: false, status: "Accepted") }
      let!(:u2_s1) { create(:algo_submission, user_id: user2.id, challenge_id: question.id, passed_test_cases: 1, total_test_cases: 10, is_best_submission: false, is_submitted: true, status: "Accepted") }
      let!(:u2_s2) { create(:algo_submission, user_id: user2.id, challenge_id: question.id, passed_test_cases: 5, total_test_cases: 10, is_best_submission: true, is_submitted: true, status: "Accepted") }
      before do
        question.regenerate_challenge_leaderboard

...
RSpec.describe UserScoreUpdate, type: :worker do
  let!(:question) do
    create(:challenge, topic: 0, question_body: 'testbody xyz', user_id: user.id, name: 'two sum test', is_active: true, score: 100,
                       input_format: [{ "name": 'n', "variable": { "dtype": 'int', "dependent": [], "datastructure": 'primitive' } }, { "name": 'arr', "variable": { "dtype": 'int', "dependent": ['n'], "datastructure": 'array' } }], output_format: [{ "name": 'out', "variable": { "dtype": 'int', "datastructure": 'array' } }])
  end
